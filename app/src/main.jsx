...
import './index.css'
      ...
      function formatPrice(price, locale = 'en') {
      ...
      function formatYear(year, locale = 'en') {
        const suffix = i18n[locale]?.['filter.yearSuffix'] || i18n['en']?.['filter.yearSuffix'] || '';
      ...
      async function detectLocale() {
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (i18n[data.browserLang]) {
              return data.browserLang;
            }
            if (i18n[data.suggestedLang]) {
      ...
      function translateGenre(genre, locale = 'en') {
        const genreKeyMap = {
          'Action': 'genre.action',
      ...
      function translateReview(review, locale = 'en') {
        const reviewKeyMap = {
      ...
      const DB_NAME = 'GameSeekerVaultDB';
      const DB_VERSION = 2;
      ...
      const dbHelper = {
        ...
        async addFavorite(folderId, gameId, sortOrder = null) {
          const db = await initDB();
        ...
        async getFavoritesByFolder(folderId) {
          const db = await initDB();
        ...
        async getFavoriteByGameId(gameId) {
          const db = await initDB();
          const tx = db.transaction(FAVORITES_STORE, 'readonly');
          const store = tx.objectStore(FAVORITES_STORE);
        ...
        async updateFavoriteFolder(favoriteId, newFolderId) {
          const db = await initDB();
          const tx = db.transaction(FAVORITES_STORE, 'readwrite');
          const store = tx.objectStore(FAVORITES_STORE);
        ...
        async updateFavoriteOrder(favoriteId, sortOrder) {
          const db = await initDB();
          const tx = db.transaction(FAVORITES_STORE, 'readwrite');
          const store = tx.objectStore(FAVORITES_STORE);
          const fav = await new Promise((resolve) => {
        ...
        async markAsDeleted(favoriteId) {
          const db = await initDB();
          const tx = db.transaction(FAVORITES_STORE, 'readwrite');
          const store = tx.objectStore(FAVORITES_STORE);
          const fav = await new Promise((resolve) => {
        ...
        async restoreFromTrash(favoriteId) {
          const db = await initDB();
          const tx = db.transaction(FAVORITES_STORE, 'readwrite');
          const store = tx.objectStore(FAVORITES_STORE);
          const fav = await new Promise((resolve) => {
      ...
      const BAND_REPRESENTATIVE_PRICE_YEN = { UNDER_5: 600, "5_10": 1200, "10_15": 1900 };
      const THEMES = {
        default: {
          name: 'デフォルト',
          bg: 'bg-gray-50',
      ...
      const translateReviewScore = (score) => {
      ...
      const checkJapaneseSupport = (supportedLanguages) => {
      ...
      const linkFor = (g) => g.storeUrl ? g.storeUrl : `https://store.steampowered.com/search/?term=${encodeURIComponent(g.title)}`;
      const appIdFromUrl = (url) => { const m = url?.match(/\/app\/(\d+)\//); return m ? m[1] : null; };

      // String cut considering character width (half-width: 1, full-width: 2)
      const truncateByWidth = (text, maxWidth) => {
        let width = 0;
        let result = '';
