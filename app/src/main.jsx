...
function SteamPriceFilter({ initialData = null }) {
  ...
  const [collectionModalScrollTop, setCollectionModalScrollTop] = React.useState(0);
  const [shiftPressedForDelete, setShiftPressedForDelete] =
    React.useState(false);
  const [isHoveringDeleteButton, setIsHoveringDeleteButton] =
    React.useState(false);
  ...
  React.useEffect(() => {
    (async () => {
      ...
      try {
        // Check if user came back via browser back button
        ...
        const isBackNavigation = navigationType === 'back_forward' ||
                                 document.referrer.includes('steampowered.com') ||
        ...
        if (isBackNavigation) {
          ...
          if (savedUIState && (Date.now() - savedUIState.timestamp < 24 * 60 * 60 * 1000)) {
            ...
            if (savedUIState.filters) {
              ...
              if (savedUIState.filters.maxPrice !== undefined) setMaxPrice(savedUIState.filters.maxPrice);
              setPriceMode(savedUIState.filters.priceMode || 'current');
              setSortOrder(savedUIState.filters.sortOrder || 'asc');
            }
            ...
            if (savedUIState.modals) {
              ...
              if (savedUIState.modals.collectionModalScrollTop !== undefined) {
                setCollectionModalScrollTop(savedUIState.modals.collectionModalScrollTop);
              }
            }
  ...
  React.useEffect(() => {
    (async () => {
      ...
      if (existingFolders.length === 0) {
        ...
        const defaultFolderId = await dbHelper.addFolder(
          t("folder.default.interested", locale),
        );
        const folder2Id = await dbHelper.addFolder(
        ...
        setFolders([
          {
            id: defaultFolderId,
            name: t("folder.default.interested", locale),
            createdAt: formatDateTime(Date.now()),
          },
          {
            id: folder2Id,
            name: t("folder.default.wishlist", locale),
            createdAt: formatDateTime(Date.now()),
          },
          {
            id: folder3Id,
        ...
        for (const folder of existingFolders) {
          if (
            folder.createdAt &&
            folder.createdAt.match(/^\d{2}-\d{2}-\d{2}/)
          ) {
  ...
  const games = React.useMemo(
    () =>
      (rawGames || []).map((g) => {
        ...
        const price =
          deal.price !== "-" && deal.price !== undefined ? deal.price : regular;
        const cut = deal.cut || 0;
        const storeLow =
          deal.storeLow !== "-" && deal.storeLow !== undefined
            ? deal.storeLow
            : "-";
  ...
  const filtered = React.useMemo(() => {
    return games.filter((g) => {
      ...
      const matchesGenre = (() => {
        ...
        const includeMatch = hasInclude
          ? selectedGenres.include.every((sel) => g.genres?.includes(sel))
          : true;
        const excludeMatch = hasExclude
          ? !selectedGenres.exclude.some((sel) => g.genres?.includes(sel))
          : true;

        return includeMatch && excludeMatch;
      })();

      // Tag filter: AND condition (all selected tags must be present)
      const matchesTags =
  ...
  // Save current UI state to IndexedDB
  const saveCurrentUIState = React.useCallback(async () => {
    try {
      const stateToSave = {
  ...
  return (
    <>
      ...
      <div
        className={`min-h-screen ${theme.bg} ${theme.text} transition-colors duration-100`}
      >
        <Header
          theme={theme}
          currentTheme={currentTheme}
        ...
        <div className="max-w-7xl mx-auto px-6 md:pt-14 pb-8 space-y-6">
          ...
          <div className="md:hidden space-y-4">
            ...
            <div className="relative w-full">
              <input
                ...
                className={`${theme.cardBg} ${theme.text} ${theme.border} border rounded-lg px-3 py-2 pr-10 text-base w-full focus:outline-none focus:ring-2 ${currentTheme === "steam" ? "focus:ring-[#4668FF]" : "focus:ring-blue-500"}`}
              />
              {searchTitle && (
                <button
          ...
          <section className="hidden md:grid grid-cols-1 md:grid-cols-5 gap-6">
            ...
            >
              ...
              <div className="flex items-center gap-4 flex-wrap mb-6">
                <div className="flex items-center gap-2">
                  <input
                    ...
                    checked={onlySale}
                    onChange={(e) =>
                      handleFilterChange(setOnlySale)(e.target.checked)
                    }
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    checked={onlyJP}
                    onChange={(e) =>
                      handleFilterChange(setOnlyJP)(e.target.checked)
                    }
                    className="h-4 w-4 flex-shrink-0"
                  />
                  <label htmlFor="jpOnly" className="text-sm whitespace-nowrap">
                    {t("filter.onlyJapanese", currentLocale)}
                  </label>
                </div>
                <div className="flex items-center gap-2">
                  <input
                    ...
                    checked={onlyMac}
                    onChange={(e) =>
                      handleFilterChange(setOnlyMac)(e.target.checked)
                    }
                    className="h-4 w-4 flex-shrink-0"
                  />
                  <label
              ...
              <div className="mt-2 space-y-2">
                ...
                <div className="mt-4 space-y-3">
                  <div>
                    <div className="text-sm font-semibold">
                      {t("filter.priceMode.title", currentLocale)}
                    </div>
                    <div
                      className={`mt-2 inline-flex flex-wrap rounded-xl ${theme.buttonBg} p-1 gap-1`}
                    >
                      <button
                        onClick={() =>
                          handleFilterChange(setPriceMode)("current")
                        }
                        className={`px-3 py-1 rounded-lg text-sm ${priceMode === "current" ? theme.buttonActive : ""}`}
                      >
                        {t("filter.priceCurrent", currentLocale)}
                      </button>
                      <button
                        onClick={() =>
                          handleFilterChange(setPriceMode)("normal")
                        }
                        className={`px-3 py-1 rounded-lg text-sm ${priceMode === "normal" ? theme.buttonActive : ""}`}
                      >
                        {t("filter.priceNormal", currentLocale)}
                      </button>
                      <button
                        onClick={() =>
                          handleFilterChange(setPriceMode)("lowest")
                        }
                        className={`px-3 py-1 rounded-lg text-sm ${priceMode === "lowest" ? theme.buttonActive : ""}`}
                      >
                        {t("filter.priceLowest", currentLocale)}
                      </button>
                      <button
                        onClick={() =>
                          handleFilterChange(setPriceMode)("discount")
                        }
                        className={`px-3 py-1 rounded-lg text-sm ${priceMode === "discount" ? theme.buttonActive : ""}`}
                      >
                  ...
                  <div>
                    <div className="text-sm font-semibold">
                      {t("filter.sortOrder", currentLocale)}
                    </div>
                    <div
                      className={`mt-2 inline-flex rounded-xl ${theme.buttonBg} p-1`}
                    >
                      <button
                        onClick={() => handleFilterChange(setSortOrder)("asc")}
                        className={`px-3 py-1 rounded-lg text-sm ${sortOrder === "asc" ? theme.buttonActive : ""}`}
                      >
                        {t("filter.sortAsc", currentLocale)}
                      </button>
                      <button
                        onClick={() => handleFilterChange(setSortOrder)("desc")}
                        className={`px-3 py-1 rounded-lg text-sm ${sortOrder === "desc" ? theme.buttonActive : ""}`}
                      >
                        {t("filter.sortDesc", currentLocale)}
                      </button>
                    </div>
                  </div>
                </div>
            ...
            <div
              className={`${theme.cardBg} ${theme.cardShadow} rounded-2xl p-4 md:col-span-3 flex flex-col`}
            >
              <div className="text-sm font-semibold">
              ...
              <div className="mt-auto flex items-center gap-2 justify-end">
                ...
                >
                  ...
                  >
                    <span className="flex-1 text-center">
                      {selectedYear === "all"
                        ...
                        : formatYear(selectedYear, currentLocale)}
                    </span>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                  ...
                  {showYearDropdown && (
                    <div
                      className={`absolute top-full z-50 w-full ${theme.cardBg} ${theme.border} border rounded-lg shadow-lg max-h-60 overflow-y-auto`}
                    >
                      <div
                        onClick={() => {
                          setSelectedYear("all");
          ...
          <div
            className={`mb-4 text-sm ${theme.subText} flex items-center justify-between gap-2`}
          >
            <span>
              {t("search.resultsCount", currentLocale).replace(
                "{count}",
        ...
        {/* Footer */}
        <footer
          className={`fixed bottom-0 left-0 right-0 z-20 ${theme.bg} px-6 py-2 transition-all duration-300 ${isScrolled ? "translate-y-full opacity-0" : "translate-y-0 opacity-100"}`}
        >
            ...
            <div className="text-center">
              <p className={`text-[9px] sm:text-xs ${theme.textSecondary}`}>
                <span className="sm:hidden">
                  Data from Steam & IsThereAnyDeal. Not affiliated.
                </span>
                <span className="hidden sm:inline">
                  Powered by Steam Web API and IsThereAnyDeal API. Not
                  affiliated with Valve or IsThereAnyDeal.
                </span>
              </p>
            </div>
          </div>
